...
while True:
    ...
    if check:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            cv2.putText(unknown_image, name, (left + 6, top - 6), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2,
                        cv2.LINE_AA)
            if matches[best_match_index]:
                name = known_face_names[best_match_index]
                makeAttendanceEntry(name, file_name)
                cv2.putText(unknown_image, "Match found, Proceeding to attention detection part", (left + 10, top - 10),
...
def cam_callibration():
    ...
    images = glob.glob('data/data/images/*.jpg')
...
while True:
            ...
            for faceLms in shape.multi_face_landmarks:
                ...
                if ear < EYE_AR_THRESH:
                    ...
                    if COUNTER >= EYE_AR_CONSEC_FRAMES:
                        ...
                        cv2.putText(img, "DROWSINESS ALERT!" + str(drowsiness_time), (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.7, (0, 0, 255), 2)
    ...
    cv2.putText(img, GAZE + " " + str(round(drowsiness_time, 2)) + " " + str(round(yawn_time, 2)), (20, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 80), 2)
    cv2.putText(img, "drowsiness_time=" + str(round(drowsiness_time, 2)) + "   yawn_time=" + str(round(yawn_time, 2)),
                (40, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 80), 2)
